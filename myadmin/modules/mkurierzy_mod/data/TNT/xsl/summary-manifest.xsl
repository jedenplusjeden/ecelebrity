...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="resourcesDir">http://iconnection.tnt.com:81/</xsl:variable>
  <xsl:variable name="cons-per-page" select="20" />
  ...
  <xsl:template match="/">
    ...
    <html>
      ...
      <body onBeforePrint="prePrint();" onAfterPrint="postPrint();">
        ...
        <xsl:apply-templates select="CONSIGNMENTBATCH" mode="browser" />
      </body>
    </html>
  ...
  <xsl:template match="CONSIGNMENT">
    <tr>
      <td><font class="data"><xsl:value-of select="CONNUMBER"/></font></td>
  ...
  <xsl:template name="totals-and-sign-off">
    ...
    <xsl:variable name="totals-row">
      <tr>
        ...
        <td align="right"><font class="data"><xsl:value-of select="count(/CONSIGNMENTBATCH/CONSIGNMENT)" /></font></td>
        <td align="right"><font class="data"><xsl:value-of select="sum(/CONSIGNMENTBATCH/CONSIGNMENT/TOTALITEMS)" /></font></td>
    ...
    <tr>
      <td colspan="8"><font class="data">Account <xsl:value-of select="HEADER/SENDER/ACCOUNT" /> Totals :</font></td>
    </tr>

    <xsl:copy-of select="$totals-row" />
    
    <tr>
      <td colspan="8"><font class="data">Sender <xsl:value-of select="HEADER/SENDER/COMPANYNAME" /> Totals :</font></td>
    </tr>
    
    <xsl:copy-of select="$totals-row" />
      
    <tr>
      <td colspan="8"><font class="data">Grand Totals :</font></td>
    </tr>
