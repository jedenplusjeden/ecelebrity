...
var ajaxCart = {
    ...
    add : function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist){
        ...
        var allChecked = true;
        var notAllMessage = '<p>Musisz wybrać: </p><ul>';
        $('.attribute_fieldset').each(function(){
            if ( $(this).find('input:checked').length <= 0 ) {
                notAllMessage += '<li>' + $(this).find('.attribute_label').text().replace(':', '') + '</li>';
                allChecked = false;
            }
        });
        notAllMessage += '</ul>';
        if ( ! allChecked ) {
            $.magnificPopup.open({
                items: {
                    src: $('#message-empty').html(notAllMessage)
                },
                type: 'inline'
            }, 0);
            return false;
...
//        $('#layer_cart').css({'top': n}).fadeIn('fast');
    ...
    updateCartEverywhere : function(jsonData){
        $('.ajax_cart_total').text($.trim(jsonData.nbTotalProducts) + ' szt.');
