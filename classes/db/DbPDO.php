...
{
	protected static function _getPDO($host, $user, $password, $dbname, $timeout = 50)
	{
		$dsn = 'mysql:';
		if ($dbname)
			$dsn .= 'dbname='.$dbname.';';
	...
	{
		try {
			$this->link = $this->_getPDO($this->server, $this->user, $this->password, $this->database, 50);
		} catch (PDOException $e) {
	...
	{
		try {
			$link = DbPDO::_getPDO($server, $user, $pwd, $db, 50);
		} catch (PDOException $e) {
			return false;
		}
	 ...
	 */
	public static function tryToConnect($server, $user, $pwd, $db, $newDbLink = true, $engine = null, $timeout = 50)
	{
		try {
	...
	{
		try {
			$link = DbPDO::_getPDO($server, $user, $pwd, false, 50);
		} catch (PDOException $e) {
			return false;
		}
